    ...
    if faceCurFrame:
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            if matches[matchIndex]:
                ...
                if counter == 0:
                    ...
                    counter = 1
                    modeType = 1
                confidence = (1 - faceDistance[matchIndex]) * 100
        ...
        if counter != 0:
            if counter == 1:
                ...
                # Data update
                datetimeObject = datetime.strptime(userinfo['last_attendance_time'],
                                                  "%Y-%m-%d %H:%M:%S")
                secondsElapsed = (datetime.now()-datetimeObject).total_seconds()
                print(secondsElapsed)
                # timer of next detection
            ...
            if modeType != 3:
                ...
                if counter <= 10:
                    ...
                    cv2.putText(imgBackground, str(userinfo['position']), (1006, 550),
                                cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 1)
                    cv2.putText(imgBackground, str(id), (1006, 493),
                                cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 1)
                    # cv2.putText(imgBackground, str(userinfo['Department']), (910, 625),
